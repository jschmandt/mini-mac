...
Each ECU saves recent messages sent on the bus \red{(only for each key group to which the ECU belongs)} and concatenates them into the HMAC input. 
...
\red{It may be necessary to instruct all ECUs to delay sending messages upon start up for a small period of time to prevent fast-booting devices from sending messages before all members of the key group are able to receive the message and maintain synchronization.}
