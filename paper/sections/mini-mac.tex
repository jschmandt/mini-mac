...
for over 30~minutes, even if messages are continuously sent using one key.
We acknowledge, however, that this choice of $\lambda$ is unreasonably large for most applications.
...
we adapted Conte's~\cite{Conte-SHA1}\footnote{\url{http://bradconte.com/sha1_c}} SHA-1~\cite{FIPS-180-4} implementation for the MSP430 platform, producing a 160-bit output. 
...
we also adapted Conte's~\cite{Conte-SHA256}\footnote{\url{http://bradconte.com/sha256_c}} SHA-2-256 implementation for the MSP430 platform, producing a 256-bit output. 
