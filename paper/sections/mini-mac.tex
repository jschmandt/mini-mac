...
\label{design}
	...
	\begin{figure}
		...
		the XOR of $M_n$, a counter~$C$, and the most recent $\lambda$ messages.}
		\label{fig-minimac}
	\end{figure}
...
We recommend setting the number of messages $\lambda$ in the message history such that
the time to collect the history exceeds the time available to a transient attacker.
For example, $\lambda = \hbox{?}$, requires the attacker to observe messages
for over 30~minutes.
