...
By creating a message history by sampling from a long enough (e.g., 30~min) stream of messages, 
no transcient attacker will have enough time to observe the entire sample.
Powerfully, this strategy defeats all transcient attackers, even if they know the group keys.
For example, this strategy defeats an attacker who learns the group keys by compromising
the key distribution process or by physically probing the ECUs, but who does not modify
the car.
...
we recommend that the message history be defined as two samples from the recent stream of valid messages
received under the given group key.  
The first sample comprises the most recent $\lambda/2$ messages taken over a specified 
recent section of the stream.  For example, one might sample every 10,000th message.  
The section length should be chosen to exceed the time available to a transcient attacker.
For example, at most 72,000 messages can be sent in 30~minutes,
even if messages are continuously sent using only one key.

The second sample comprises the most recent $\lambda/2$ 
messages from this stream.  
This sample helps ensure changed state after a resynchronization (see Section~\ref{resynch}).
