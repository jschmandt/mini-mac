...
the frequency with which messages can be sent on the CAN bus [what is this frequency?  It is derived from
the 500 kbs stated earlier.  in table, change b to bits]
...
%% it is a bit lame to emphasize that we are optional and woo is not.
	...
	assuming the adversary injects $X$ forged messages per minute.}
	\label{tab-taglength}
	\vspace{8pt}
	\begin{tabular}{c|r}%
