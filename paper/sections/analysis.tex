...
an intentional tradeoff to ensure that Mini-MAC does not delay any message. The only 8-byte
messages that we observed were status reports.  
It may be possible to reduce the length of such status data slightly 
by deleting some of the low-order data bits.

Because all messaage types have fixed, known message lengths, the adversary cannot 
